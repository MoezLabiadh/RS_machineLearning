...
#img_path = os.path.join (workspace, 'inputs','S2_band_composite.tif')
shp_path = os.path.join (workspace, 'inputs','training_data.shp')


#-------------------------------------------------------#
# STEP 1: Prepare input data: imagery and training dataset
#-------------------------------------------------------#
...
img_path = os.path.join (workspace, 'inputs','S2A_' + os.path.basename(Gpath)[:15] + '_Band_Composite.tif')

outds = gdal.BuildVRT(outvrt, S2_bands, separate=True)
outds = gdal.Translate(img_path, outds)
...
roi_raster_path = os.path.join (workspace, 'outputs','training_data.tif')
driver = gdal.GetDriverByName('GTiff')
roi_raster_ds = driver.Create(roi_raster_path, 
                              ncol, 
                              nrow, 
                              1, 
                              gdal.GDT_Byte)
...
#Open the ROI raster    
roi_ds = gdal.Open(roi_raster_path) 
